...
评分维度说明：
- writing_score：文笔看表达、描写、语言流畅度评价
- logic_score：逻辑看设定自洽、伏笔回收、人物行为动机
- update_speed_score：更新看更新频率、拖更、断更、完结状态
- character_score：人物塑造看角色深度、立体感、成长弧线
- toxicity_index：毒性指数综合粉丝互撕、控评水军、恶意攻击等负面社区行为，0 表示健康，1 表示极度恶化
评论没提到的维度给 5 到 6 分，toxicity_index 没提到给 0.15-0.25，不要编造事实。
"""
...
CHARACTER_POSITIVE = ("人物", "角色", "群像", "立体", "丰满", "鲜活", "人设")
CHARACTER_NEGATIVE = ("人设崩", "扁平", "脸谱化", "工具人", "配角", "标签化")
TOXICITY_KEYWORDS = ("控评", "水军", "互撕", "饭圈", "引战", "刷分")
...
def analyze_reviews(
    ...
    if batch_delay is None:
        batch_delay = float(os.getenv("LLM_BATCH_DELAY", "8"))
    agent_batches = 0
    heuristic_batches = 0
...
def analyze_batch_with_agent_resilient(
    ...
    if retry_delay is None:
        retry_delay = float(os.getenv("LLM_BATCH_DELAY", "8"))
    try:
...
def heuristic_analysis(review: Review) -> ReviewAnalysis:
    ...
    character_score = dimension_score(text, list(CHARACTER_POSITIVE), list(CHARACTER_NEGATIVE))
    toxicity_index = coerce_score(
        sum(1.0 for kw in TOXICITY_KEYWORDS if kw in text) * 0.22,
        0.0, 1.0, 0.15,
    )
...
def build_report(
    ...
    score_fields = {
        ...
        "character": [item.character_score for item in analyses],
        "toxicity": [item.toxicity_index for item in analyses],
