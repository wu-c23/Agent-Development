...
from sentiment_critic.qidian import collect_qidian_reviews
from sentiment_critic.models import write_jsonl


def main() -> None:
    parser = argparse.ArgumentParser(description="采集/导入起点、豆瓣、贴吧、小红书等平台的小说深度书评。")
    parser.add_argument("--book", required=True, help="小说名，例如：诡秘之主")
    parser.add_argument(
        "--platform",
        action="append",
        choices=["qidian", "douban", "tieba", "xiaohongshu"],
        help="要搜索的平台，可重复；不填时默认抓 qidian + douban",
    )
    ...
    platforms = args.platform or ["qidian", "douban"]
    ...
    elif platforms == ["qidian"] and not args.url:
        reviews = collect_qidian_reviews(
            book=args.book,
            pages=args.max_pages,
            limit=args.limit,
            min_chars=args.min_chars,
