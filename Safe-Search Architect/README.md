...
```bash
python -m src.safe_search --mode api --host 0.0.0.0 --port 8002
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/v1/search/vector/health` | 向量库健康检查 + 索引统计 |
| `POST` | `/api/v1/search/semantic` | 语义搜索 + 避雷过滤 |
| `POST` | `/api/v1/search/books/index` | 批量添加/更新书籍索引 |
| `GET` | `/api/v1/search/books/{uid}` | 查询单本书 + 实时风险评分 |
| `DELETE` | `/api/v1/search/books/{uid}` | 从索引中删除书籍 |
| `GET` | `/api/v1/search/intent?query=...` | 意图解析（调试用） |
...
```bash
curl -X POST http://localhost:8002/api/v1/search/books/index \
...
```bash
curl http://localhost:8002/api/v1/search/vector/health
# → {"status":"healthy","version":"1.0.0","index_count":2,"last_updated":"..."}
```
...
```
Safe-Search Architect/
├── .env                                # 环境变量（API Key 等，已 gitignore）
├── .env.example                        # 环境变量模板
├── requirements.txt                    # Python 依赖
├── README.md                           # 本文件
├── safe-search-agent-prompts.md        # Few-shot 避雷过滤指令集
│
├── data/
...
│
├── docs/
│   ├── vector-db-schema.md             # 向量数据库 Schema 文档 v2.0
...
└── src/
    ...
    │   ├── __main__.py                 # 统一入口（--mode api|mcp，默认端口 8002）
    │   ├── api.py                      # FastAPI REST 服务（符合 Data Contract）
    │   ├── config.py                   # 配置（环境变量加载）
    ...
    │   ├── mock_server.py              # Mock API 服务（联调用，端口 8002）
    │   ├── models.py                   # Book / IntentResult 数据模型
    │   ├── uid_utils.py                # 系统 UID 生成工具
