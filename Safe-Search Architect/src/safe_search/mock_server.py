...
_RUNS_DIR = _ROOT / "Sentiment Critic" / "data" / "runs"


def _build_review_data_titles() -> set[str]:
    """扫描 runs/ 目录，提取已有评论数据的书名集合。"""
    titles: set[str] = set()
    if not _RUNS_DIR.exists():
        return titles
    for f in _RUNS_DIR.iterdir():
        if not f.suffix == ".jsonl":
            continue
        name = f.stem
        for marker in ("_tieba_", "_douban_", "_zongheng_"):
            if marker in name:
                titles.add(name.split(marker)[0])
                break
    return titles


_REVIEW_DATA_TITLES = _build_review_data_titles()
...
def _build_mock_novels() -> list[dict]:
    ...
    for item in central_data:
        ...
        novels.append({
            ...
            "has_review_data": title in _REVIEW_DATA_TITLES,
            "safe_check": {
...
# 确保所有小说都有 has_review_data 字段（原始精选 Mock 可能缺失）
for _novel in MOCK_NOVELS:
    if "has_review_data" not in _novel:
        _novel["has_review_data"] = _novel["metadata"]["title"] in _REVIEW_DATA_TITLES
...
async def semantic_search(request: SearchRequest):
    ...
    for novel in MOCK_NOVELS:
        ...
        # 优先推荐有评论数据的书：有数据 +0.12，无数据略降
        if novel.get("has_review_data"):
            relevance += 0.12
        else:
            relevance -= 0.03
        ...
        results.append({
            ...
            "has_review_data": novel.get("has_review_data", False),
