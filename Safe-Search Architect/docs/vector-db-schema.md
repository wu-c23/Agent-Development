...
根据项目数据契约，全系统统一 UID = `SHA256(Platform_ID + "|" + Novel_Title)`。在 SafeSearch 模块中：
- `Book.id` 字段使用此 UID 作为值
- 若创建 Book 时未提供 id，系统自动通过 `platform_id + title` 生成
- 向量索引的 ChromaDB document ID = Book.id = 系统 UID
- 这样搜索结果可直接通过 UID 与其他模块（舆情评分、趋势数据）关联
- 完整 UID 规范见 System Integrator 的 `uid-specification.md`

### Metadata 字段补充

| 字段 | 类型 | 说明 |
|------|------|------|
| `platform` | str | 来源平台标识: qidian / zongheng / douban / ... |

此字段在 v1.0 中新增，用于 Data Contract 的 metadata.platform 输出。
