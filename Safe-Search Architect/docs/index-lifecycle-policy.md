...
curl -X POST http://localhost:8002/api/v1/search/books/index \
  ...
  -d '{
    "books": [
      {"title": "诡秘之主", "platform": "qidian", "platform_id": "qidian:1001001", "intro": "...", "tags": ["克苏鲁", "悬疑"], "status": "completed"},
      {"title": "修罗武神", "platform": "qidian", "platform_id": "qidian:2002002", "intro": "...", "tags": ["玄幻", "后宫"], "status": "ongoing"}
    ]
  }'
```
...
```bash
curl -X DELETE http://localhost:8002/api/v1/search/books/{uid}
```
...
# → {"status": "healthy", "index_count": 150, "last_updated": "..."}
```
...
2. 重新批量导入全部书籍：`POST /api/v1/search/books/index`
...
- API 延迟：监控 `POST /api/v1/search/semantic` 响应时间
